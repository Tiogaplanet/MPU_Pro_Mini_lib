 ...
 *          decodes hex-ASCII response strings from MiP, and routes out-of-band
 * events.
 */
class MiP_Serial {
public:
  /** @brief Minimum delay in milliseconds required between consecutive UART
   * requests sent to MiP. */
  static constexpr uint8_t MIP_REQUEST_DELAY = 8;

  /** @brief Time in milliseconds to wait for a response from MiP before timing
   * out. */
  static constexpr uint8_t MIP_RESPONSE_TIMEOUT = 100;

  /** @brief Flag for transportSendRequest() indicating no response is expected
   * from MiP. */
  static constexpr bool MIP_EXPECT_NO_RESPONSE = false;

  /** @brief Flag for transportSendRequest() indicating a response is expected
   * from MiP. */
  ...
  /** @brief Maximum buffer length for incoming response buffers received from
   * MiP. */
  static constexpr size_t MIP_RESPONSE_MAX_LEN = 5 + 1;  // Longest response is
                                                         // MIP_CMD_REQUEST_CHEST_LED.
   ...
   * (fire-and-forget).
   *
   * @details Used for asynchronous commands or internally by higher-level
   * verified methods.
   *
   ...
   * @param requestLength       Number of bytes in the request buffer.
   * @param responseBuffer     Buffer where received response data from MiP will
   * be written.
   * @param responseBufferSize Maximum capacity of the response buffer.
   * @param responseLength     Reference receiving the actual number of response
   * bytes written.
   * @return uint8_t Status code (returns MIP_ERROR_NONE on success, or an error
   * code on timeout/failure).
   */
  uint8_t rawReceive(
...
private:
  ...
  /**
   * @brief Waits for and reads a response frame matching the expected command
   * from MiP.
   * @param pResponseBuffer    Target memory location for response data.
   * @param responseBufferSize Size of destination buffer.
   * @param pResponseLength   Pointer where total byte count read will be
   * stored.
  ...
  /**
   * @brief Sends a request packet to MiP over hardware serial, enforcing delay
   * timing.
  ...
  uint8_t m_expectedResponseSize;  ///< Size in bytes of expected response from
                                   ///< MiP
